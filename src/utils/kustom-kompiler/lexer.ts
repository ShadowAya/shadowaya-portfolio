...
const tokenSpecification = [
    ...
    ['LVAR', '#[a-zA-Z_]*'],       // Local Variables
    ['GVAR', '@[a-zA-Z_]*'],       // Global Variables
...
export default function lexer(code: string) {
    const tokens: {
        tokens: { kind: TokenKinds; value: string, line: number, column: number }[],
        error?: {
            line: number,
            column: number
        }
    ...
    const tokenRegex = new RegExp(
        tokenSpecification.map(([kind, pattern]) => `(?<${kind}>${pattern})`).join('|'),
        'g'
      );
