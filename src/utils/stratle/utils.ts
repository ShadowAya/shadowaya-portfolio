...
export async function getStratagemList() {
    ...
    const stratagemLinks = doc.find("//table[contains(@class, 'wikitable')][1]/tbody/tr/td[2]/a/@href")
        .map(a => (
            "https://helldivers.wiki.gg" +
            (a.child(0)?.text()??'')
        ) as `https://helldivers.wiki.gg${string}`);
    ...
    for (const link of stratagemLinks) {
        ...
        const table = doc.find("//aside[contains(@class, 'portable-infobox')][1]")[0];
        const icon = doc.find("//aside[contains(@class, 'portable-infobox')][1]//img[@class='pi-image-thumbnail']/@src")[0].child(0)?.text()??null;
        const content = table.find(".//div[contains(@class, 'pi-data-value')]");

        const statsTable = doc.find("//table[contains(@class, 'wikitable')]/tbody")[0].childNodes().map(e => e.text());

        let usesString = statsTable.find(a => a.startsWith('\nUses'))??'';
        usesString = (usesString.includes('Standard') ?
            /^(.|\n)+Standard(\n)+(?<res>.+)(\n)+$/g.exec(usesString)?.groups?.res :
            /^(.|\n)+Uses(\n)+(?<res>.+)(\n)+$/g.exec(usesString)?.groups?.res)??'Unlimited';
        const uses = usesString === 'Unlimited' ? 0 : parseInt(usesString.replace(' seconds', ''));

        let cooldownString = statsTable.find(a => a.startsWith('\nCooldown'))??'';
        cooldownString = /^(.|\n)+Standard(\n)+(?<res>.+)(\n)+$/g.exec(cooldownString)?.groups?.res??'Unlimited';
        const cooldown = cooldownString === 'Unlimited' ? 0 : parseInt(cooldownString.replace(' seconds', ''));

        const entry = (content.length == 6 ? {
            ...
            code: content[5].find(".//img/@src").map(a =>
                parseArrow(a.toString().replace(/^\s+src="/, '').replace(/"$/, ''))
            ),
        } : {
            permitType: content[0].child(0)?.text(),
            unlockLevel: '1',
            unlockCost: content[1].find(".//span")[0].child(0)?.text().replace(',', ''),
            module: 'Warbond',
        ...
        entry.name = table.get('h2')?.toString().replace(/<[^>]+>/g, '')??'';
        entry.icon = icon;
    ...
    const missionStratagems = doc.find("//table[contains(@class, 'wikitable')][2]/tbody/tr")
        .slice(1)
        .map(a => a.childNodes());
    ...
    const missionStratagemsParsed = missionStratagems.map(a => {
        ...
        const entry: Stratagem = {
            ...
            module: 'Mission',
            traits: [lastTrait],
            code: a[5].find(".//img/@src").map(n => parseArrow(n.child(0)?.text()??'')),
            uses: name in usesMap ? usesMap[name as keyof typeof usesMap] : 0,
            cooldown: name in cooldownMap ? cooldownMap[name as keyof typeof cooldownMap] : 0,
        };
        return entry;
    });
...
export async function getStratagemListCached() {
    ...
    if (Date.now() - data.updatedAt > 1000 * 60 * 60 * 24) { // 1 day
        getStratagemList();
    }
