...
export async function getStratagemList() {
    ...
    console.log("💀 Fetched Stratagems page");
    // const html = fs.readFileSync('./content.txt', 'utf8');

    const doc = await libxmljs.parseHtmlAsync(html);

    console.log("💀 Getting stratagem links");
    const stratagemLinks = doc
        .find(
            "//table[contains(@class, 'wikitable')][1]/tbody/tr/td[2]/a/@href"
        )
        .map(
            (a) =>
    ...
    console.log(`💀 Found ${stratagemLinks.length} stratagem links`);

    let counter = 1;
    for (const link of stratagemLinks) {
        const res = await fetch(link, { cache: "no-store" });
        console.log(`  💀 ${counter++} Fetching stratagem: ${link}`);
        const html = await res.text();
        ...
        do {
            ...
            !statsTable.some((a) => a.includes("Uses")) &&
            statsTableI < statsTableList.length
        );
        ...
        cooldownString =
            /^(.|\n)+Standard(\n)+(?<res>.+)(\n)+$/g.exec(cooldownString)
                ?.groups?.res ?? "Unlimited";
        const cooldown =
    ...
    const missionStratagemsParsed = missionStratagems.map((a) => {
        ...
        const entry: Stratagem = {
            ...
            cooldown:
                name in cooldownMap
                    ? cooldownMap[name as keyof typeof cooldownMap]
                    : 0,
        };
        return entry;
    });
