...
export default function MoreInfo() {
  ...
  useEffect(() => {
    ...
    function handleBrowserFullscreenChange() {
      ...
      const isExitingBrowserFullscreen =
        isBrowserFullscreenRef && !isWindowFullscreen;
  ...
  return (
    <div className={styles.more}>
      ...
      <div className={styles.right}>
        ...
        <button className={styles.withPopup}>
          <Iconify
          ...
          />
          <div className={cn(showSettings && styles.show)}>
            <Settings />
          </div>
        </button>
        <button className={styles.withPopup}>
          ...
          <div className={cn(showInfo && styles.show)}>
            <div>
              <h4></h4>
              <span>Daily stratagems rotate on UTC+0 midnight</span>
            </div>
            <div>
              <h4></h4>
              <span>This website is primarily designed for desktop use</span>
            </div>
            <div>
              <h4>Credit</h4>
              <span>
                <a target="_blank" href={"https://helldivers.wiki.gg/"}>
                  helldivers.wiki.gg
                </a>{" "}
                - Stratagem Data <br />
                <a
...
function InstallPrompt({
  ...
  const [manualInstallString, setManualInstallString] = useState<string>(
    "Install this app inside your browser's menu"
  );
  ...
  useEffect(() => {
    if (
      ...
      setManualInstallString(
        "Tap the share button and select 'Add to Home Screen'"
      );
    } else if (/Android/.test(navigator.userAgent)) {
      setManualInstallString(
        "Tap the menu button and select 'Install' or 'Add to Home Screen > Install'"
      );
  ...
  return (
    <button
      ...
      onClick={canActivate ? handleInstallClick : handleManualInstallClick}
    >
      <Iconify icon="material-symbols:download" height={40} />
